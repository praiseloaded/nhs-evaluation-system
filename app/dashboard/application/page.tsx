...
function StatementBuilderWizardInner() {
  ...
  // ── Load existing application when ?id=xxx&step=N is in the URL ──────────
  useEffect(() => {
    const urlId   = searchParams.get('id')
    const urlStep = searchParams.get('step')
    if (!urlId) return

    // Load the application data and jump to the specified step
    const loadExisting = async () => {
      try {
        ...
        // Restore all application context into wizard state
        ...
        if (a.criteria?.length) {
          const list: Criterion[] = a.criteria.map((c: any) => ({
            id: c.id,
          ...
          // Mark already-saved evidence
          const saved: Record<string, boolean> = {}
          a.criteria.forEach((c: any) => {
            if (c.generatedParagraph) saved[c.id] = true
          })
          setSavedEvidence(saved)

          // Find first unanswered criterion to start at
          const firstUnanswered = a.criteria.findIndex((c: any) => !c.generatedParagraph)
          setEvidenceIndex(firstUnanswered >= 0 ? firstUnanswered : 0)
        }

        // Restore context fields if saved
        if (a.currentRole) setCurrentRole(a.currentRole)

        // Jump to the requested step
        const targetStep = urlStep ? Number(urlStep) : 4
        setStep(targetStep)
        setMode('wizard')
      } catch (err) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (s.step && s.step > 1) { setStep(s.step); setMode('wizard') }
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-md">
                Build evidence-based NHS supporting statements for Scotland, England, Wales and Northern Ireland.
              </p>
            </div>
            {mode === 'list' && (
              <button
            ...
            {mode === 'wizard' && step > 1 && (
              <button onClick={clearWizard}
                className="shrink-0 text-xs text-muted-foreground hover:text-foreground border border-border rounded-lg px-3 py-2 hover:bg-accent transition-colors">
                ← All Statements
              </button>
            )}
          </div>
          ...
          {mode === 'list' && !loadingPast && totalApps > 0 && (
            <div className="flex items-center gap-3 mt-5 flex-wrap">
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800">
                ...
                <span className="text-xs text-emerald-600 dark:text-emerald-400">statement{generatedCount !== 1 ? 's' : ''} generated</span>
              </div>
              {inProgressCount > 0 && (
          ...
          <div className="space-y-3">
            {loadingPast ? (
              ...
              <div className="rounded-2xl border border-dashed border-border bg-card/50 p-12 text-center space-y-5">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                    Upload a job description to extract the criteria and build your first NHS supporting statement.
                  </p>
                </div>
                <button
                  onClick={() => { setMode('wizard'); setStep(1) }}
              ...
              pastApps.map((a, idx) => {
                ...
                const href = `/dashboard/application/${a.id}`
                  ...
                  <Link key={a.id} href={href}
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {a.band && (
                          <span className="shrink-0 text-[10px] font-semibold bg-muted text-muted-foreground px-2 py-0.5 rounded-full">
                            {a.band}
                          </span>
                        )}
                        {appNation !== 'unknown' && (
                          <span className={`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${nMeta.color}`}>
                            {nMeta.flag} {nMeta.label}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        {a.employer && (
                          <span className="text-xs text-muted-foreground flex items-center gap-1">
                            <Globe className="w-3 h-3" /> {a.employer}
                          </span>
                        )}
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 ${
                          ...
                          {isGenerated ? (
                            <><CheckCircle2 className="w-2.5 h-2.5" /> Ready to submit</>
                          ) : (
                            <><Clock className="w-2.5 h-2.5" /> In progress</>
                          )}
                        </span>
                        {a.wordCount ? (
                          <span className="text-[10px] text-muted-foreground">{a.wordCount} words</span>
                        ) : null}
                      </div>
                      ...
                      <div className="h-1 bg-muted rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${isGenerated ? 'bg-emerald-500' : 'bg-primary/60'}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
