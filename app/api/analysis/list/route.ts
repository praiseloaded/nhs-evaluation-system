...
export async function GET(req: NextRequest) {
  ...
  if (!session?.user?.id) {
    return Response.json({ success: false, error: 'Unauthorised', results: [] }, { status: 401 })
  }
  ...
  try {
    ;[analyses, total] = await db.$transaction([
      db.analysis.findMany({
        ...
        skip,
        take:    limit,
        select:  { id: true, jobTitle: true, result: true, createdAt: true },
      }),
    ...
    return Response.json({ success: false, error: 'Failed to fetch analyses', results: [] }, { status: 503 })
  }
  ...
  const results = analyses.map(a => {
    ...
    if (!raw.scoredBreakdown && raw.breakdown) {
      try { raw.scoredBreakdown = calculateNhsBandScore(raw as any) }
      catch (e) { console.warn(`[ANALYSES_GET] Failed to score ${a.id}:`, e) }
    }
    ...
    return {
      id:                   a.id,
      jobTitle:             a.jobTitle,
      band:                 (raw.band     as string | undefined) ?? null,
