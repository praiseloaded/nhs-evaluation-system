...
function buildKeywordPrompt(statement: string, jobDescription: string, keywordsToCheck: typeof NHS_KEYWORD_TAXONOMY): string {
...
{
  "keywords": [
    { "keyword": "Clinical Governance", "status": "present_evidenced", "evidence": "brief description", "recommendation": "what to add if missing" }
  ],
  "overallKeywordScore": 72,
  "criticalMissing": ["keyword1"],
  "quickWins": ["add X"]
}`.trim()
}
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await callGeminiJSON(buildKeywordPrompt(statement, jobDesc, NHS_KEYWORD_TAXONOMY), 3000)
    ...
    try {
      await db.analysis.update({ where: { id }, data: { keywordIntel: responseData } })
    } catch (e) {
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return Response.json({ success: true, data: analysis.keywordIntel ?? null })
  } catch (error: any) {
    return Response.json({ error: error?.message ?? 'Failed' }, { status: 500 })
  }
}