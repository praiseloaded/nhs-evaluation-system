...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const evidenceEntries = await prisma.evidenceEntry.findMany({ where: { userId: session.user.id } })
    const competencies = await prisma.competency.findMany({ where: { userId: session.user.id } })
...
`.trim()
    ...
    try {
      ...
      await prisma.analysis.update({ where: { id }, data: { interviewProbability: responseData } })
    } catch (saveErr) {
