...
function RegisterForm() {
  ...
  // Read params passed by the "Analyse this job" button on /jobs and /job/cos
  ...
  // After successful registration, sign the user in and redirect:
  // - If jobUrl present → go straight to analysis/new with job pre-filled
  // - If returnTo present → honour it
  // - Otherwise → dashboard
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Auto sign-in after registration so user lands logged-in
  ...
  // Google sign-in also carries job params through callbackUrl
  function handleGoogle() {
    const callbackUrl = getPostRegisterUrl()
  ...
  return (
    <div className="w-full lg:w-[60%] max-w-4xl flex border border-[#1E3A5F] rounded-2xl overflow-hidden">
      ...
      <div className="flex-1 bg-[#0A1628] px-8 py-10">
        ...
        <div className="mb-6">
          ...
          <p className="text-[12px] text-[#6B8FAE] leading-relaxed">
            {jobTitle
              ? <>Register to analyse <strong className="text-[#B8D4EC]">"{jobTitle}"</strong> and get your instant shortlist score.</>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[13px] font-medium text-[#4A6A8A] mb-1.5 tracking-wider">FIRST NAME</label>
              <input type="text" required placeholder="Jane" value={firstName} onChange={e => setFirstName(e.target.value)} className={inp} />
            </div>
            <div>
              <label className="block text-[13px] font-medium text-[#4A6A8A] mb-1.5 tracking-wider">LAST NAME</label>
              <input type="text" required placeholder="Smith" value={lastName} onChange={e => setLastName(e.target.value)} className={inp} />
            </div>
          </div>

          {/* Email */}
          <div>
            <label className="block text-[13px] font-medium text-[#4A6A8A] mb-1.5 tracking-wider">EMAIL ADDRESS</label>
            <input type="email" required placeholder="jane@nhs.net" value={email} onChange={e => setEmail(e.target.value)} className={inp} />
          </div>

          {/* Password */}
          <div>
            <label className="block text-[13px] font-medium text-[#4A6A8A] mb-1.5 tracking-wider">PASSWORD</label>
          ...
          <button type="submit" disabled={loading}
            className="w-full flex items-center justify-center gap-2 bg-blue-700 hover:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg py-2.5 text-[13px] font-medium transition-colors mt-1">
            <UserPlus className="w-3.5 h-3.5" />
            {loading ? "Creating account..." : jobTitle ? `Register & analyse "${jobTitle.slice(0, 30)}${jobTitle.length > 30 ? '…' : ''}"` : "Create account"}
          </button>
        ...
        <button onClick={handleGoogle}
          className="w-full flex items-center justify-center gap-2 bg-[#060D1F] border border-[#1E3A5F] hover:bg-[#0D2240] hover:border-[#2A5080] rounded-lg py-2.5 text-[12px] font-medium text-[#B8D4EC] transition-all">
        ...
        <p className="text-center text-[13px] text-[#4A6A8A] mt-4">
          ...
          <Link href={`/login${jobUrl ? `?jobUrl=${encodeURIComponent(jobUrl)}&jobTitle=${encodeURIComponent(jobTitle)}` : ''}`} className="text-blue-400 hover:underline">
            Sign in
          </Link>
        </p>
      ...
      <div className="hidden lg:flex w-70 flex-col justify-center bg-[#060D1F] border-l border-[#1E3A5F] px-6 py-10 shrink-0">
        <p className="text-[13px] font-medium tracking-widest text-blue-400 mb-2">WHY OMNI</p>
        <h2 className="text-[16px] font-semibold text-[#E2EEF9] mb-2 leading-snug">The unfair advantage for NHS applicants</h2>
        <p className="text-[13px] text-[#4A6A8A] mb-6 leading-relaxed">AI trained on NHS recruitment — not generic job boards.</p>
...
export default function RegisterPage() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#060D1F] px-6 py-12">
      <Suspense fallback={
        <div className="w-full max-w-4xl h-[500px] bg-[#0A1628] border border-[#1E3A5F] rounded-2xl animate-pulse" />
      }>
        <RegisterForm />
      </Suspense>
    </div>
